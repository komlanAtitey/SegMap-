...
FSV_RANGE = (0.60, 0.95)                                  # true FSV of spatial genes
K_NN, M_BASIS = 30, 100                                   # graph k, spatial basis size
...
def simulate_discrete_domains(
    ...
    domains[(coords[:,0] >= 0.5) & (coords[:,1] < 0.5)] = 1
    domains[(coords[:,0] < 0.5) & (coords[:,1] >= 0.5)] = 2
    domains[(coords[:,0] >= 0.5) & (coords[:,1] >= 0.5)] = 3

    n_spatial = int(round(frac_spatial * G))

    is_spatial = np.zeros(G, dtype=bool)
    is_spatial[:n_spatial] = True
    ...
    for j in range(G):
        ...
        if is_spatial[j]:
            ...
            domain_signal = mu[domains]

            domain_signal -= domain_signal.mean()
            domain_signal /= (
                domain_signal.std() + 1e-8
            )
...
def main():
    ...
    coords, expr, is_sp, fsv_true, genes, domains = \
        simulate_discrete_domains(
            N,
            G,
            FRAC_SPATIAL,
            FSV_RANGE,
