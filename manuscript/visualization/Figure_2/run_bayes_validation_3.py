...
# =============================== simulation ================================ #
# ============================================================================
# Generation of cell-type-confounded spatial expression data
# ============================================================================

def simulate_celltype_confounded(
        N,
        G,
        frac_spatial,
        lengthscale,
    ...
    # --------------------------------------------------------
    # Spatially localized cell-type niches
    # --------------------------------------------------------
    centers = rng.random((n_celltypes, 2))

    dist = np.zeros((N, n_celltypes))
    ...
    celltype = np.argmin(dist, axis=1)
    ...
    # Gene classes
    # --------------------------------------------------------
    n_spatial = int(round(frac_spatial * G))

    n_confounded = int(
        round(frac_confounded * G)
    )
    ...
    gene_class[:n_spatial] = "spatial"

    gene_class[
        n_spatial:
        n_spatial + n_confounded
    ] = "celltype"

    rng.shuffle(gene_class)

    expr = np.empty((N, G))

    fsv_true = np.zeros(G)

    is_spatial = np.zeros(G, dtype=bool)
    ...
    for j in range(G):
        ...
        # ----------------------------------------------
        # CELL-TYPE-CONFOUNDED GENE
        # ----------------------------------------------
        elif gene_class[j] == "celltype":

            celltype_effects = rng.normal(
                0,
                2,
                n_celltypes
            )
...
def main():
    ...
    coords, expr, is_sp, fsv_true, genes, gene_class, celltype = (
        simulate_celltype_confounded(
              N,
              G,
              FRAC_SPATIAL,
              LENGTHSCALE,
