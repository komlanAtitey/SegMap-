...
# ------------------------------- CONFIG ------------------------------------- #
N, G, FRAC_SPATIAL = 1500, 300, 0.30

# stationary model (baseline)
LENGTHSCALE = 0.10

# nonstationary simulation
LENGTHSCALE_LEFT  = 0.08 # localized structure
LENGTHSCALE_RIGHT = 0.18 # broad smooth gradients

NONSTATIONARY_FRACTION = 1.0
...
# left half = short-range correlation
# right half = long-range correlation
# ======================================================================

# ======================================================================
# Nonstationary covariance construction
# ======================================================================

def build_nonstationary_covariance(
        coords,
        ell_left=0.05,
        ell_right=0.25,
        jitter=1e-5):
    ...
    """
    Positive-definite nonstationary covariance.

    Correlation length changes smoothly across the tissue.
    Left side = short-range structure.
    Right side = long-range structure.
    """
    ...
    # smooth spatially varying lengthscale
    ell = (
        ell_left +
        (ell_right - ell_left) *
        x
    ...
    # Paciorek-Schervish nonstationary kernel
    prefactor = np.sqrt(
        (2 * ell_i * ell_j) /
        (ell_i**2 + ell_j**2)
    )
    ...
    K = prefactor * exponent

    K = 0.5 * (K + K.T)

    eigmin = np.min(np.linalg.eigvalsh(K))

    if eigmin < 0:
        K += (-eigmin + jitter) * np.eye(K.shape[0])
    else:
...
def simulate_nonstationary(
    ...
    K_nonstat = build_nonstationary_covariance(
        coords,
        ell_left=LENGTHSCALE_LEFT,
        ell_right=LENGTHSCALE_RIGHT
    )

    L_nonstat = np.linalg.cholesky(K_nonstat)

    n_sp = round(frac * G)

    is_sp = np.zeros(G, dtype=bool)
    is_sp[:n_sp] = True
    ...
    for j in range(G):
        ...
        if is_sp[j]:
            ...
            spatial_component -= spatial_component.mean()

            spatial_component /= (
                np.std(spatial_component) + 1e-8
            )
...
def main():
    ...
    coords, expr, is_sp, fsv_true, genes = (
        simulate_nonstationary(
            N,
            G,
            FRAC_SPATIAL,
            FSV_RANGE,
    ...
    summary = pd.DataFrame([{
        "simulation":
        "Nonstationary_Gaussian_Misspecification",
